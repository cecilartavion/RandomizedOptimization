...
hiddens_wine 
alphas = [10**-x for x in np.arange(-2,5.01,1)]
#alphas = alphas[0:3]
alphas 
...
#                for train, validate in kf.split(wine_train):
#                    np.random.seed(55)
#                    train_data = wine_train.iloc[train].copy()
#                    val_data = wine_train.iloc[validate].copy()
#                    train_dataX = train_data.loc[:,train_data.columns!='quality'].copy()
#                    train_dataY = train_data['quality'].copy()
#                    val_dataX = val_data.loc[:,val_data.columns!='quality'].copy()
#                    val_dataY = val_data['quality'].copy()
...
#                    y_train_pred = nn_model1.predict(train_dataX)
#                    y_train_accuracy = accuracy_score(train_dataY,y_train_pred)
#                    temp_train_acc.append(y_train_accuracy)
#                    print('Training accuracy is ',y_train_accuracy, ' with learning rate ', 
#                          alpha, ', layers ',layer,', activation function,', 
#                          activation,', and algorithm ', algorithm)
#                    y_val_pred = nn_model1.predict(val_dataX)
...
#                          alpha, ', layers ',layer,', activation function,', 
#                          activation,', and algorithm ', algorithm)
#                    temp_val_acc.append(y_val_accuracy)
...
pd.DataFrame(alg_time).to_csv('./output/alg_times.csv')
rhc = df[df['algorithm']=='random_hill_climb']
rhc.to_csv('./output/random_hill_climb_wine_main.csv')  
sa = df[df['algorithm']=='simulated_annealing']
sa.to_csv('./output/simulated_annealing_wine_main.csv')  
ga = df[df['algorithm']=='genetic_alg']
ga.to_csv('./output/genetic_alg_wine_main.csv')  
#mim = df[df['algorithm']=='mimic']
#mim.to_csv('./output/mimic_wine_main.csv')  

best_param_rhc = ['sigmoid',1,[24,24]]
best_param_sa = ['relu',10,[24,24]]
best_param_ga = ['relu',100,[24,24]]

wine_trainX = wine_train.loc[:,wine_train.columns!='quality'].copy()
wine_trainY = wine_train['quality'].copy()
wine_testX = wine_test.loc[:,wine_test.columns!='quality'].copy()
wine_testY = wine_test['quality'].copy()
nn_model1 = mlrose.NeuralNetwork(best_param_rhc[2],activation=best_param_rhc[0],algorithm='random_hill_climb',max_iters=1000,
                                 bias=False,is_classifier=True,learning_rate=best_param_rhc[1])
...
print('Training accuracy is ',y_train_accuracy, ' with learning rate ', 
      alpha, ', layers ',layer,', activation function,', 
      activation,', and algorithm ', algorithm)
y_val_pred = nn_model1.predict(wine_testX)
y_val_accuracy = accuracy_score(wine_testY,y_val_pred)
print('Testing accuracy: ',y_val_accuracy, ' with learning rate ', 
      alpha, ', layers ',layer,', activation function,', 
      activation,', and algorithm ', algorithm)
###################################
...
###################################
###################################
nn_model1 = mlrose.NeuralNetwork(best_param_sa[2],activation=best_param_sa[0],algorithm='random_hill_climb',max_iters=1000,
                                 bias=False,is_classifier=True,learning_rate=best_param_sa[1])
nn_model1.fit(wine_trainX,wine_trainY)
y_train_pred = nn_model1.predict(wine_trainX)
y_train_accuracy = accuracy_score(wine_trainY,y_train_pred)
y_train_accuracy
...
for pct in np.arange(0,1,0.1):
    samp = wine_train.sample(n=int(pct*len(wine_train)), random_state=42)
###################################
###################################
###################################
###################################
###################################
nn_model1 = mlrose.NeuralNetwork(best_param_ga[2],activation=best_param_ga[0],algorithm='random_hill_climb',max_iters=1000,
                                 bias=False,is_classifier=True,learning_rate=best_param_ga[1])
nn_model1.fit(wine_trainX,wine_trainY)
y_train_pred = nn_model1.predict(wine_trainX)
y_train_accuracy = accuracy_score(wine_trainY,y_train_pred)
y_train_accuracy
